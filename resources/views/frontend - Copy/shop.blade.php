...
@php
    ...
    if($category_id != null){
        ...
        $variants      = App\Models\Variants::where('category_id',$category_id)->get();
        $category_name = App\Models\Category::where('id',$category_id)->pluck('category_name')->first();

    } else {

       $category_id   = App\Models\Variants::where('id',$variant_id)->pluck('category_id')->first();
       $variants      = App\Models\Variants::where('category_id',$category_id)->get();
       $category_name = App\Models\Category::where('id',$category_id)->pluck('category_name')->first();
       $variant_name  = App\Models\Variants::where('id',$variant_id)->pluck('name')->first();
                   ...
                   <div class="breadcrumb__list">
                    ...
                    @else
                        <span style="margin-left:20%;margin-right:30%;padding-top:90%">
                            <b>{{ $category_name}}</b> @if($variant_id != null) - {{ $variant_name}}  @endif
                        </span>
                    @endif
                   ...
                   <div class="tp-shop-widget mb-50">
                      ...
                      <div class="tp-shop-widget-content">
                         <div class="tp-shop-widget-categories">
                            <ul>
                                @php  $all = App\Models\VariantOptions::where('is_deleted','<>','1')->count('variant_options.id'); @endphp

                                <li > <a href="{{ route('frontend.shop')}}" >All <span>  {{$all}} </span></a>  </li>

                                @foreach ($categories as $category)

                                    @php  $count = App\Models\VariantOptions::where('is_deleted','<>','1')->where('category_id',$category['id'])->count('variant_options.category_id'); @endphp

                                  <li > <a href="{{ route('frontend.shop',['id'=>$category['id']]) }}"> {{ ucfirst(strtolower( $category->category_name)) }} <span>  {{$count}} </span></a>
                    ...
                    @php
                    $featuredproducts = App\Models\VariantOptions::where('is_active','=',1)->where('is_deleted','<>',1)->where('is_featured','=',1)->orderBy('id','desc')->limit(4)->get();
                    @endphp
                        ...
                        @endphp
                            ...
                            <div class="tp-shop-widget-product-item d-flex align-items-center">
                               <div class="tp-shop-widget-product-thumb">
                                  <a href="{{ route('frontend.single_product',['id'=>$products->id]) }}">
                                     <img src="{{ asset('backend/images/product_images/'.$products->image) }}" width="70px" height="70px" alt="">
                                  </a>
                               </div>
                               <div class="tp-shop-widget-product-content">

                                  <h4 class="tp-shop-widget-product-title">
                                     <a href="product-details.html"> {{ Str::limit($products->name, 20)}} </a>
                                  </h4>
                                  <div class="tp-shop-widget-product-price-wrapper">
                                     <span class="tp-shop-widget-product-price"> ₹{{ $min_amount }} </span>
                                  </div>
                               </div>
                            </div>
                         @endforeach
                         ...
                         <div class="col-xl-6">
                            <div class="tp-shop-top-left d-flex align-items-center ">
                               ...
                               <div class="tp-shop-top-result">
                                  <p>Showing {{ $variant_options->firstItem() }}–{{ $variant_options->lastItem() }} of {{ $variant_options->total() }} results</p>
                               </div>
                            </div>
                         </div>
                      ...
                      <div class="tab-content" id="productTabContent">
                         <div class="tab-pane fade show active" id="grid-tab-pane" role="tabpanel" aria-labelledby="grid-tab" tabindex="0">
                            ...
                            @foreach ($variant_options as $options)
                                ...
                                $pid            = $options->id;
                                $min_amount     = App\Models\ProductAmountOptions::where('is_deleted','<>',1)->where('product_id','=', $pid)->min('amount');
                                $min_amount_id  = App\Models\ProductAmountOptions::where('is_deleted','<>',1)->where('product_id','=', $pid)->where('amount','=', $min_amount)->pluck('id')->first();
                                $qty_name   = App\Models\ProductAmountOptions::where('is_deleted','<>',1)->where('id','=', $min_amount_id)->pluck('qty_name')->first();
                                @endphp
                                ...
                                <div class="col-xl-4 col-md-6 col-sm-6 infinite-item">
                                    ...
                                    <div class="tp-product-thumb-2 p-relative z-index-1 fix w-img">
                                        <a href="{{ route('frontend.single_product',['id'=>$options['id']]) }}">
                                           <img src="{{ asset('backend/images/product_images/'.$options->image) }}" width="261px" height="278px" alt="">
                                        </a>
                                        <!-- Add to cart -->
                                        <div class="tp-product-action-2 tp-product-action-blackStyle">
                                           <div class="tp-product-action-item-2 d-flex flex-column">

                                            <form action="{{ route('cart.store', ['id' => $options->id]) }}" method="post">
                                                @csrf
                                                <input type="hidden" name="product_id" id="product_id" value="{{$options->id}}">
                                              ...
                                              <form action="{{ route('wishlist.store', ['id' => $options->id]) }}" method="post">
                                                @csrf

                                                <input type="hidden" name="product_id" id="product_id" value="{{$options->id}}">
                                                <input type="hidden" name="product_qty" id="product_qty" value="1">

                                              <button type="submit" class="tp-product-action-btn-2 tp-product-add-to-wishlist-btn">
                                     ...
                                     <div class="tp-product-content-2 pt-15">
                                        <div class="tp-product-tag-2">
                                           @if($options->variant_id != null){{ $variant_nameARR[$options->variant_id] }}  @endif
                                        </div>
                                        <h3 class="tp-product-title-2" style="font-size:16px;">
                                           <a href="{{ route('frontend.single_product',['id'=>$options['id']]) }}"> {{  $options->name }}</a>
                                        </h3>

                                        <div class="tp-product-price-wrapper-2">
                                           <span class="tp-product-price-2 new-price">{{ $qty_name }} - ₹{{ $min_amount }} </span>
                                    ...
                                    @php $variant_option_id = $options->id;  @endphp
                                     ...
                                     <div class="tp-product-list-item d-md-flex">
                                        <div class="tp-product-list-thumb p-relative fix">
                                           <a href="{{ route('frontend.single_product',['id'=>$options['id']]) }}">
                                              <img src="{{ asset('backend/images/product_images/'.$options->image) }}" style="width: 350px; height:310px;" alt="">
                                           </a>
                                        ...
                                        <div class="tp-product-list-content">
                                           <div class="tp-product-content-2 pt-15">
                                              ...
                                              <div class="tp-product-price-wrapper-2">
                                                 <span class="tp-product-price-2 new-price"> ₹1500 </span>
           ...
           <div class="modal-content">
              <div class="tp-product-modal-content d-lg-flex align-items-start">
                 ...
                 <div class="tp-product-details-wrapper">
                    ...
                    <h3 class="tp-product-details-title" id="modal-product-price">{{ $options->name }}</h3>
